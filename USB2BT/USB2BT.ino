...
void loop()
{
  ...
  if (buttonPressed == true && millis() - pressedTime >= holdTime) {
    ...
    // Do something if the button released before the threshold time, like displaying battery status from the LED
  }
  //-----------------------------------------------------------------//
  Usb.Task();
...
void buttonEvent() {                            // Whenever a button interrupt occurs, the buttonEvent function will call
